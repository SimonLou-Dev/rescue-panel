        ...
        return (
            <div className={'ContentCard'}>
                ...
                <div className={"item-list"}>
                    ...
                    this.state.items.map((item) =>
                        <div className={'item'} key={item.id}>
                            ...
                            {this.props.type === 4 &&
                            <button style={{display: this.display(item.id)}} onClick={this.delete}><img alt={""}
                                                                                                        data={this.state.type + '_' + item.id}
                                                                                                        src={rootUrl + (item.deleted_at !== null ? 'assets/images/invisibility.svg' : 'assets/images/visibility.svg')}/>
                            </button>
                            }
                            {this.props.type !== 4 && !(this.props.type === 10 && item.id === 1) &&
                            <button style={{display: this.display(item.id)}} onClick={this.delete}><img alt={""}
                                                                                                        data={this.state.type + '_' + item.id}
                                                                                                        src={rootUrl + 'assets/images/cancel.png'}/>
                            </button>
                            }
                        </div>
                    )
                ...
                <form method={"POST"} onSubmit={this.post}>
                    <input type={"text"} value={this.state.formcontent} maxLength={"30"} onChange={(e) => {
                        this.setState({formcontent: e.target.value})
                    }}/>
                    {this.props.type === 8 &&
                    <input type={'number'} value={this.state.price} onChange={(e) => {
                    ...
                    {this.props.type === 9 &&
                    <input type={'text'} value={this.state.color} placeholder={'#ffffff'} onChange={(e) => {
